...
from simxarm.task.base import Base
from simxarm.task.reach import Reach
from simxarm.task.push import Push
from simxarm.task.peg_in_box import PegInBox
from simxarm.task.lift import Lift
...
class SimXarmWrapper(gym.Wrapper):
	...
	def __init__(self, env, task, obs_mode, image_size, action_repeat, frame_stack=1, channel_last=False):
		...
		image_shape = (image_size, image_size, 3*frame_stack) if channel_last else (3*frame_stack, image_size, image_size)
		if obs_mode == 'state':
