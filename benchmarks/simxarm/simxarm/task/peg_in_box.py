...
class PegInBox(Base):
	...
	def _get_obs(self):
		...
		obs = np.concatenate([
			eef, eef_velp, box, obj, obj_rot, obj_velp, obj_velr,
			eef-box, eef-obj, obj-box,
			np.array([
				np.linalg.norm(eef-box), np.linalg.norm(eef-obj),
				np.linalg.norm(obj-box), gripper_angle
			])
		], axis=0)
		return dict(
			observation=obs,
